...
public class SearchMinimumNumber
{
    ...
    {
        return binarySearchForMinimum(arr, 0, arr.length - 1);
    }

    private static int binarySearchForMinimum(int[] arr, int first, int last)
    {
        int middle = first + (last - first) / 2;

        if(arr[last] < arr[middle])
        {
            return binarySearchForMinimum(arr, middle + 1, last);
        }
        else if (arr[last] > arr[middle])
        {
            return binarySearchForMinimum(arr, first, middle);
        }
        else
        {
            return first;
